...
--
-- > myLayout = ThreeCol 1 (3/100) (1/2) (1/2) ||| ThreeColMid 1 (3/100) (1/3) (1/2) ||| etc..
-- > main = xmonad def { layoutHook = myLayout }
--
-- This is the same as the normal ThreeCol with the only difference
-- being that a second fraction is added. This is the fraction that is
-- used if there are two layouts.
--
...
-- | Arguments are nmaster, delta, fraction, fraction_two_columns
data ThreeCol a = ThreeColMid { threeColNMaster :: !Int, threeColDelta :: !Rational, threeColFrac :: !Rational, threeColFrac2 :: !Rational }
                | ThreeCol    { threeColNMaster :: !Int, threeColDelta :: !Rational, threeColFrac :: !Rational, threeColFrac2 :: !Rational }
    deriving (Show,Read)

instance LayoutClass ThreeCol a where
    pureLayout (ThreeCol    n _ f f2) r s = doL False n (pickf n f f2 s) r s
    pureLayout (ThreeColMid n _ f f2) r s = doL True  n (pickf n f f2 s) r s
    handleMessage l m =
        return $ msum [fmap resize     (fromMessage m)
                      ,fmap incmastern (fromMessage m)]
            where resize Shrink = l { threeColFrac = func f, threeColFrac2 = func f2 }
                    where func x = max (-0.5) $ x-d
                  resize Expand = l { threeColFrac = func f, threeColFrac2 = func f2 }
                    where func x = min 1 $ x+d
...
pickf :: Int -> Rational -> Rational -> W.Stack a -> Rational
pickf master f1 f2 s | len == master + 1 = f2
                     | otherwise          = f1
  where len = length $ W.integrate s
