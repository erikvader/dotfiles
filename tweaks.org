...
runs on the other filesystems, like home, so a 2 needs to be added in fstab for those, and
a 0 for the root system so it doesn't get checked twice. I think that is how it works.
** PC speaker as bell
